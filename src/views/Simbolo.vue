    ...
    div.simbolo-view
        ...
        div.container.col-lg-8.p-0
            section.form-inline
                ...
                button.btn.btn-green(:disabled="simboloInvalido" @click="salvarSimbolo") Salvar

            section.col-md-12.p-0.mt-3
                div.col-md-12.p-0
                ...
                div.col-md-12.pt-4.pb-2.px-0.row.m-0
                    ...
                    div.col-md-4.p-0
                        ul.simbolo-preview(v-if="simbolo && simbolo.categoria")
                            li
                                simbolo.simbolo-lg(:simbolo="simbolo" preview=true)
                            li
                                simbolo.simbolo-sentenca(:simbolo="simbolo" preview=true)
    ...
    methods: {
        ...
        salvarSimbolo() {
            ...
            axios({
                ...
                if(response.status == 201){
                    toastr.success('Símbolo criado com sucesso.');
                }
            })
            .catch(error => {
                this.showLoader(false);
                ...
                if (error.response) {
                    toastr.error(error.response.data.message, 'Erro ao criar símbolo');
                }
                else {
                    toastr.error(error.message, 'Erro ao criar símbolo');
                }
            });
        },
        ...
        previewArquivo (event) {
            if (event.target.files && event.target.files[0]) {
                ...
                if(!tipo) {
                    toastr.error("Arquivo inválido.", "Apenas arquivos de imagem e vídeo com as seguintes extensões são permitidos: .jpg, .jpeg, .png, .gif, .mp4");
                    event.target.value = "";
                    return;
                }
                ...
                switch(this.simbolo.tipo) {
                    case process.env.VUE_APP_SIMBOLO_IMAGEM:
                        if(FileUtils.toMB(file.size) > process.env.VUE_APP_IMG_FILE_SIZE_MB){
                            toastr.error("O tamanho do arquivo ultrapassa o permitido.", "Arquivos de imagem são limitados em " + process.env.VUE_APP_IMG_FILE_SIZE_MB + "MB. Utilize um compressor como o <a href=\"https://tinyjpg.com\" target=\"_blank\">Tiny JPG</a> para reduzir o tamanho do arquivo.");
                            event.target.value = ""
                            return;
                        }
                    ...
                    case process.env.VUE_APP_SIMBOLO_VIDEO:
                        if(FileUtils.toMB(file.size) > process.env.VUE_APP_VIDEO_FILE_SIZE_MB){
                            toastr.error("O tamanho do arquivo ultrapassa o permitido.", "Arquivos de vídeo são limitados em " + process.env.VUE_APP_VIDEO_FILE_SIZE_MB + "MB. Utilize um compressor como o <a href=\"https://www.youcompress.com/\" target=\"_blank\">YouCompress</a> para reduzir o tamanho do arquivo.");
                            event.target.value = ""
                            return;
                        }
